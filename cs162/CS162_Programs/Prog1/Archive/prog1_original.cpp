	...
	else
	{
	    ...
	    do
	    {
	        //Prompt and read in the length of the commercial 
 	        cout << "Please enter the length of a commercial you plan to air (in minutes): ";
		...
		else
		{
		    do
		    {
			//Prompt and read in the number of times the commercial with be aired
		        cout << "How many times do you plan on airing this commercial? ";
		        cin >> comm_num;
		        comm_total = comm_length * comm_num;
		        if (comm_total < 0 || comm_total > total_time)
			    cout << "The number you entered is out of range...please try again!";
		        else
		        {
		            total_time -= comm_total;
			    if (total_time > 0)
			    {
				...
				{
				    cout << "The character you entered is not valid..."
