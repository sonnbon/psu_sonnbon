...
int cell_survey::set_use(char user_name[], char use_reason[], float frequency,
		         cell_usage & one_use)
{
    if (!user_name || !use_reason) return 0;
    if (one_use.user_name) delete [] one_use.user_name;
    if (one_use.use_reason) delete [] one_use.use_reason;
...
{
    if (!app_head) return 0;
    if (!strcmp(app_head->app_data.name, name_match))
    {
      use_node * use_temp = NULL;
      app_node * app_temp = NULL;
      while (use_temp)
      {
        use_temp = app_head->use_head->next_use;
	delete app_head->use_head;
	app_head->use_head = use_temp;
      }
