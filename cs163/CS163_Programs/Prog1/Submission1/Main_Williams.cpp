...
int main()
{
    ...
    float frequency;        //To be temporary float for user's usage frequency
                            //(in hours per day)
    char response;          //To be user's response yes or no
    int user_input;         //To be user's option choice and/or test choice
    ...
    do
    {
      ...
      if (!app_list.set_use(user_name, use_reason, frequency, one_use))
        cerr << "Something went wrong... Input not accepted...\n\n";
    ...
    if(!app_list.display_usage(type))
      cerr << "Thre is nothing in the list...\n\n";
