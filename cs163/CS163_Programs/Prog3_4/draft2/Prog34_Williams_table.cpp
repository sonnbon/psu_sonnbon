...
int table::hash_function(char key_value1[], char key_value2[])
{
  ...
  length1 = strlen(key_value1); //Length of key_value1
  length2 = strlen(key_value2); //Length of key_value2

  //Iterate through key_value1 and add each character ASCII value
  for (int i = 0; i < length1; ++i)
    sum += key_value1[i]; //Gets sum total of character values

  //Iterate through key_value2 and add each character ASCII value
  for (int i = 0; i < length2; ++i)
    sum += key_value2[i]; //Gets sum total of character values
...
{
  int index = hash_function(type, breed); //Gets the index to search the table
  ...
  while (current)
  {
    ...
    current = current->next; //Current gets the value of its next ponter
  }
                      ...
                      const animal & add_animal)
{
  ...
  if (!root)
  {
    ...
    root->left = NULL;                 //Set root's left pointer to NULL
    root->right = NULL;                //Set root's right pointer to NULL

    return 1; //Returns a success of the operation
  }


  //Check if objects data is complete.
  if (!root->new_animal.get_sorted(breed, name))
    return 0; //Returns a failure of the operation

  //Check if inputted data is less than the object's data, alphabetically
  if (root->new_animal.get_sorted(breed, name) == 1)
  {
    //Call the function recursively, passing in the root's left pointer
    return BST_insert(root->left, breed, name, add_animal);
  }

  //Call the function recursively, passing in the roots' right pointer
...
int table::hash_remove_all(h_node ** & hash_table)
{
  ...
  for (int i = 0; i < hash_table_size; ++i)
  {
    ...
    while (temp)
    {
      temp = temp->next;    //Temp gets the value of its next pointer
      delete hash_table[i]; //Delete Hash Table's element
      hash_table[i] = temp; //Set Hash Table's element to temp
    }
  }
  delete [] hash_table; //Delete the Hash Table array
  hash_table = NULL;    //Set the Hash Table array to NULL
