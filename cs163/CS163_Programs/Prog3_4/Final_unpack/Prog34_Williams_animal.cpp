...
// a float ("age") and five dynamically allocated arrays ("type", "breed",
...
// member function passes in a float and five arrays to be set to the class'
// data members. The "copy" member function passes in an "animal" object to be 
// set and copied to another object. The "ismatch", "retrieve", and 
// "get_sorted" functions pass in two arrays of characters to find specific
// matches ("ismatch" and "retrieve") and to choose a sorting direction, 
// respectively. The "find_unique" member function passes in three arrays of
// characters in order to find virtually unique animals. All member functions 
...
int animal::retrieve(char find_breed[], char find_name[])
{
  ...
  return 0; //Returns false, the data is not a match
}



// This member function compares client inputted ata to an object's data and
// checks whether it is a unique match in value alphabetically. This member 
// function takes three arrays of characters as arugments and returns and 
// integer to represent a success or failure of the operation.
int animal::find_unique(char find_type[], char find_breed[], char find_name[])
{
  //Check that the object has valid data.
  if (!type || !breed || !name)
    return 0; //Returns a failure of the operation

  //Check if the passed in data matches the object's type, breed, and name,
  //alphabetically.
  if (!strcmp(find_type, type) && !strcmp(find_breed, breed) && 
      !strcmp(find_name, name))
    return 1; //Returns true that the data is a match
