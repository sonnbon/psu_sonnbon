...
// and copying of one animal entry by passing in arrays, a float, or an 
// object as arguments inputted from the client. For debugging purposes, it can
// display an animal entry. It also can find specific and unique animal matches
// by passing in two to three arrays of characters, respectively, and can help
// decide alphabetical value comparisons when comparing object data to two 
// passed in arrays of characters.
class animal
{
  public:
    ...
    int ismatch(char type[],         //Checks for type and breed matches
                char breed[]);
    int isempty() const;             //Checks if user inputted data is valid
    int get_sorted(char add_breed[], //Returns how object should be sorted
                   char add_name[]);
    int retrieve(char find_breed[],  //Retrieves matches by breed and name
                 char find_name[]);
    int find_unique(char find_type[],//Finds unique animal matches
                    char find_breed[],
                    char find_name[]);
