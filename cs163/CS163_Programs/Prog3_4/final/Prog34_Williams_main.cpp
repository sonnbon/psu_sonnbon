...
// The list of animals can be displayed by a particular type and breed or 
// altogether in sorted order by name and breed. The user can also find
...
int main()
{
  ...
  char filename[SIZE];  //User inputted filename to load or save externally
  int list_size = 0;    //List size of loaded or saved external file
  ...
  cout << "This application will allow you to match people who want to\n"
       ...
       << "remove, find, and display animals. You can also load animals\n"
  ...
  do
  {
    ...
    switch (menu_option)
    {
      ...
      case 6:
        //Take client input to remove an animal
        cout << "Please enter the animal's name: ";
        cin.get(name, SIZE, '\n');
        cin.ignore(400, '\n');

        cout << "Please enter the animal's type (cat, dog, bird, etc.): ";
        cin.get(type, SIZE, '\n');
        cin.ignore(400, '\n');
  
        cout << "Please enter the animal's breed (e.g., German Shepherd): ";
        cin.get(breed, SIZE, '\n');
        cin.ignore(400, '\n');

        if (!roster.remove_animal(type, breed, name))
          cerr << "Match not found...\n\n";
        else
      ...
      case 7:
        //Check and load an animal list from an external file
        cout << "Please enter the file you would like to load from: ";
        cin.get(filename, SIZE, '\n');
        cin.ignore(400, '\n');

        list_size = roster.load(filename, type, breed, name, needs, age, notes,
                                new_animal);
        if (!list_size)
          cerr << "No file exists or file is empty...\n\n";
        else
        {
          cout << list_size << " animal(s) loaded to the system." 
               << endl << endl;
        }
        break;
