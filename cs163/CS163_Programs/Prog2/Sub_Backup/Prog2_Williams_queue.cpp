...
int queue::copy(const queue & to_copy)
{
  ...
  q_node * front = to_copy.rear->next; //Set front to the value of the passed
                                       //in object's rear's next pointer
  q_node * current = front;            //Set current to the value of front

  do
  {
    enqueue(current->new_task);        //Add one task from the passed in object
