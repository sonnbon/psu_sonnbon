...
int main()
{
    ...
    do
    {
     ...
     //Is there an entry we would like to retrieve (by title)?
     cout <<"Enter a title which you would like to retrieve: ";
     cin.get(title, SIZE, '\n'); cin.ignore(SIZE,'\n');
     if (entry.retrieve(title, entry2))
     {
        cout <<"\n\nWe found:\n"; 
        if (!entry.display())
     ...
     if(!entry2.display())
         cerr << "We were unable to retrieve...\n";
