...
//This constant member function checks to see if a match is found, searching by
//"environment" type. It takes a constant pointer to a char, to match with the 
//'a_type' data member, passed in as an argument and returns a bool value.
bool card::find_type(const char * type) const
{
  //Check if the passed in data type is not NULL and that it matches the 
  //respective data member before returning true.
  if (type && !strcmp(a_type, type))
    return true; 
