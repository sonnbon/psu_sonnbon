...
// on vehicle type specific aspects of an object (vehicle, car, and truck). The 
// purpose of the vehicle class, being derived from address, is to manage all 
// vehicle information, including its make (Ford, Toyota, etc.), model (RAV4, 
// Civic, Prius, etc.), license plate number, and its type (personal or 
...
//
// The purpose of the car class, being a class derived from vehicle
// (in the Program1_Williams_location.h file), is to manage a car's specific 
// information, including the number of passengers, the car's passenger 
// capacity, and what the trip's purpose is (Restaurant, Shopping, Sight-Seeing, 
// etc.) while also inheritting from the vehicle class. The car subclass allows 
// the user to focus in more on the internal behaviour and characteristics of 
// the object. With this class, the user will be able to add a car, add or 
// remove passengers, check if a car is full of passengers, change the trip's 
// purpose, and display the object's information. 
//
// The purpose of the truck class is to manage a truck's specific information, 
// including its number of packages and its capacity while also inheritting 
// from the vehicle class. The truck subclass allows the user to focus in more 
// on the internal behaviour and characteristcs of the object, just as with the 
// car class. With this class, the user will be able to add a truck, add or 
// remove a package, check if the truck is full of packages, change the truck's 
// capacity, and display an object's information. In this hierarchy, the goal is 
// for the car or truck classes, derived from the vehicle class, to be created 
// and navigated to and from parking stalls to occupy within a city.
// *****************************************************************************
