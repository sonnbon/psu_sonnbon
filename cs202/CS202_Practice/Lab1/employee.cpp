...
hourly_employee::hourly_employee(const person & input): employee(input), hours(0), max_hours_allowed(0), overtime_ok(false), pay_rate(0.0)
{
...
void employee::copy_tree(node * & dest_root, node * source_root) const
{
    ...
    dest_root = new node(*source_root); //dereference the pointer and you end up with a node "is a" performance review object
